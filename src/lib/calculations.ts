...
export function calculateEquityScores(): EquityScore[] {
  const data = GOVERNORATE_HEALTH.map(gh => {
    ...
    const avgOccupancy = hospitals.length > 0
      ? hospitals.reduce((sum, h) => sum + h.occupancyRate, 0) / hospitals.length
      : 0;

    return {
      code: gh.governorateCode,
      bedsPerTenK: (totalBeds / totalPop) * 10000,
      // hospitalDensity: government hospitals per 100K population (used as proxy since doctor counts are unavailable per governorate)
      hospitalDensity: (gh.govtHospitals / totalPop) * 100000,
      nursesProxy: (gh.healthCentres + gh.extendedHC) / totalPop * 100000,
  ...
  const nurseValues = data.map(d => d.nursesProxy);
  const pharmValues = data.map(d => d.pharmaciesPerTenK);
  const occValues = data.map(d => 100 - d.avgOccupancy); // invert: lower occupancy = better

  return data.map(d => {
    const bedScore = normalize(d.bedsPerTenK, Math.min(...bedValues), Math.max(...bedValues));
    // hospitalDensityScore: measures hospital density per capita (proxy for doctor availability)
    const doctorScore = normalize(d.hospitalDensity, Math.min(...docValues), Math.max(...docValues));
    const nurseScore = normalize(d.nursesProxy, Math.min(...nurseValues), Math.max(...nurseValues));
    const accessScore = normalize(d.pharmaciesPerTenK, Math.min(...pharmValues), Math.max(...pharmValues));
    const stressScore = normalize(100 - d.avgOccupancy, Math.min(...occValues), Math.max(...occValues));
